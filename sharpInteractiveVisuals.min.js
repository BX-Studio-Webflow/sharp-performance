...
let threshold=brightestValue*sensitivity;for(let y=0;y<img.height;y++){for(let x=0;x<img.width;x++){let index=(x+y*img.width)*4;let r=img.pixels[index];let g=img.pixels[index+1];let b=img.pixels[index+2];let brightness=0.299*r+0.587*g+0.114*b;if(brightness>=threshold){brightestPixels.push([x+(width*0.6-img.width/2),y+(height*0.6-img.height/2),brightness]);}}}
...
effectImageCanvasPG.image(imgs[currentImg],width*0.6-imgs[currentImg].width/2,height*0.6-imgs[currentImg].height/2);effectImageCanvasPG.loadPixels();if(currentImg<totalImages-1){currentImg+=1;}else{currentImg=0;}}
