let DEBUG=false;let capacity=1;let particles=[];let numParticles=1000;let maxParticlesConnections=30;let imgs=[];let totalImages=7;let currentImg=0;let img;let brightestPixels;let randomBrightPixel;let finalImageCanvasPG;let amountOfDots=0;let effectImageCanvasPG;let lastSwitchTime=0;let interval=9500;let startTransition=false;let transition=0;let videoElement;let isMobileDevice=false;let horizontalPosition=0.7;let verticalPosition=0.6;function preload(){isMobileDevice=/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);console.log('Is Mobile Device:',isMobileDevice);if(!isMobileDevice){for(let i=1;i<=totalImages;i++){img=loadImage('/wip/v3/img00'+i+'.jpeg');imgs.push(img);}}
else{videoElement=createVideo("/wip/v3/SharpCharacter.mp4",muteVideo);}}
...
function findBrightestPixels(img,sensitivity=0.2){print(windowWidth)
if(windowWidth>1920)
img.resize(1280,0)
print(img.width,img.height)
img.loadPixels();let brightestValue=0;let brightestPixels=[];for(let y=0;y<img.height;y++){for(let x=0;x<img.width;x++){let index=(x+y*img.width)*4;let r=img.pixels[index];let g=img.pixels[index+1];let b=img.pixels[index+2];let brightness=0.299*r+0.587*g+0.114*b;if(brightness>brightestValue){brightestValue=brightness;}}}
let threshold=brightestValue*sensitivity;for(let y=0;y<img.height;y++){for(let x=0;x<img.width;x++){let index=(x+y*img.width)*4;let r=img.pixels[index];let g=img.pixels[index+1];let b=img.pixels[index+2];let brightness=0.299*r+0.587*g+0.114*b;if(brightness>=threshold){brightestPixels.push([x+(width*horizontalPosition-img.width/2),y+(height*verticalPosition-img.height/2),brightness]);}}}
...
videoElement.volume(0);videoElement.hideControls();videoElement.autoplay(true);videoElement.play();}
...
effectImageCanvasPG.image(imgs[currentImg],width*horizontalPosition-imgs[currentImg].width/2,height*verticalPosition-imgs[currentImg].height/2);effectImageCanvasPG.loadPixels();if(currentImg<totalImages-1){currentImg+=1;}else{currentImg=0;}}
function windowResized(){print("Resize canvas!")
if(navigator.userAgent.indexOf("HeadlessChrome")==-1){resizeCanvas(windowWidth,windowHeight);if(!isMobileDevice){finalImageCanvasPG.resizeCanvas(windowWidth,windowHeight);effectImageCanvasPG.resizeCanvas(windowWidth,windowHeight);brightestPixels=findBrightestPixels(imgs[currentImg]);updateImage();}
}}
let cursorX,cursorY;let outterX,outterY;let mouseXPos,mouseYPos;let maxMouseCapture=150;let mouseActive=0;let orbitingParticles=[];function initCursor(){mouseXPos=width/2;mouseYPos=height/2;cursorX=mouseXPos;cursorY=mouseYPos;outterX=mouseXPos;outterY=mouseYPos;initOrbitingParticles();}
